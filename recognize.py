#!/usr/bin/env python3
import argparse  # This should be at the top of your script with the other imports
import sys
...
def predict_chessboard(chessboard_img_path, options={}):
    ...
    _save_output_html(chessboard_img_path, predicted_fen, [p[1] for p in predictions], confidence)
    print("Saved {} prediction to {}".format(chessboard_img_path, OUT_FILE))
    print("Predicted FEN: {}".format(predicted_fen))  # Make sure this is before the return statement
...
def predict_tile(tile_img_data):
    ...
    index_of_max_probability = probabilities.index(max_probability)
    return (FEN_CHARS[index_of_max_probability], max_probability)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("-q", "--quiet", help="Suppress most output", action="store_true")
    parser.add_argument("-d", "--debug", help="Save debug output to HTML file", action="store_true")
    parser.add_argument("image_path", help="Path to a single PNG chessboard image", nargs='?', default=r'C:\Users\wheth\Desktop\DEVELOPING\chessboard-recognizer-master\chessboard_png\chessboard.png')
    ...
    # Directly use the provided image path for prediction
    predicted_fen = predict_chessboard(args.image_path, args)
    print("Predicted FEN: {}".format(predicted_fen))
